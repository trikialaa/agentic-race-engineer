...
def get_race_report(capture: F1TelemetryCapture) -> dict[str, Any]:
    ...
    participants_raw = capture.data.get("participants") or {}
    _p = (participants_raw.get("participants") or []) if isinstance(participants_raw, dict) else []
    participants: list[Any] = list(_p) if isinstance(_p, (list, tuple)) else []
    ...
    entries: list[dict] = []
    _cd = final.get("classificationData") if final else None
    classification_data: list[Any] = list(_cd) if isinstance(_cd, list) else []
    if final and classification_data:
        num_cars = final.get("numCars", 22)
        for i, c in enumerate(classification_data[:num_cars]):
            ...
            for s in range(min(num_stints, len(stints_visual))):
                ...
                tyre_stints.append(
                    {"compound": compound, "endLap": end_lap if end_lap != 255 else None}
                )
