...
def decode_final_classification(buf: memoryview):
    offset = _HDR_SIZE
    num_cars = struct.unpack_from("<B", buf, offset)[0]
    offset += 1
    classifications = []

    for _ in range(22):
        position = struct.unpack_from("<B", buf, offset)[0]
        offset += 1
        numLaps = struct.unpack_from("<B", buf, offset)[0]
        offset += 1
        gridPosition = struct.unpack_from("<B", buf, offset)[0]
        offset += 1
        points = struct.unpack_from("<B", buf, offset)[0]
        offset += 1
        numPitStops = struct.unpack_from("<B", buf, offset)[0]
        offset += 1
        resultStatus = struct.unpack_from("<B", buf, offset)[0]
        offset += 1
        resultReason = struct.unpack_from("<B", buf, offset)[0]
        offset += 1
        bestLapTime = struct.unpack_from("<I", buf, offset)[0]
        offset += 4
        totalRaceTime = struct.unpack_from("<d", buf, offset)[0]
        offset += 8
        penaltiesTime = struct.unpack_from("<B", buf, offset)[0]
        offset += 1
        numPenalties = struct.unpack_from("<B", buf, offset)[0]
        offset += 1
        ...
        classifications.append(
            {
                ...
                "resultReasonName": RESULT_REASON_NAMES.get(
                    resultReason, f"Unknown ({resultReason})"
                ),
