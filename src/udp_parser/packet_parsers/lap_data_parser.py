...
from constants import DRIVER_STATUS, PIT_STATUS, RESULT_STATUS, SECTORS
...
def decode_lap_data(buf: memoryview):
    ...
    for car_idx in range(22):
        last_lap = struct.unpack_from("<I", buf, offset)[0]
        offset += 4
        current_lap = struct.unpack_from("<I", buf, offset)[0]
        offset += 4
        sector1_ms = struct.unpack_from("<H", buf, offset)[0]
        offset += 2
        sector1_min = struct.unpack_from("<B", buf, offset)[0]
        offset += 1
        sector2_ms = struct.unpack_from("<H", buf, offset)[0]
        offset += 2
        sector2_min = struct.unpack_from("<B", buf, offset)[0]
        offset += 1
        delta_front_ms = struct.unpack_from("<H", buf, offset)[0]
        offset += 2
        delta_front_min = struct.unpack_from("<B", buf, offset)[0]
        offset += 1
        delta_leader_ms = struct.unpack_from("<H", buf, offset)[0]
        offset += 2
        delta_leader_min = struct.unpack_from("<B", buf, offset)[0]
        offset += 1
        lap_distance = struct.unpack_from("<f", buf, offset)[0]
        offset += 4
        total_distance = struct.unpack_from("<f", buf, offset)[0]
        offset += 4
        safety_car_delta = struct.unpack_from("<f", buf, offset)[0]
        offset += 4
        car_position = struct.unpack_from("<B", buf, offset)[0]
        offset += 1
        current_lap_num = struct.unpack_from("<B", buf, offset)[0]
        offset += 1
        pit_status = struct.unpack_from("<B", buf, offset)[0]
        offset += 1
        num_pit_stops = struct.unpack_from("<B", buf, offset)[0]
        offset += 1
        sector = struct.unpack_from("<B", buf, offset)[0]
        offset += 1
        current_lap_invalid = struct.unpack_from("<B", buf, offset)[0]
        offset += 1
        penalties = struct.unpack_from("<B", buf, offset)[0]
        offset += 1
        total_warnings = struct.unpack_from("<B", buf, offset)[0]
        offset += 1
        corner_cutting_warnings = struct.unpack_from("<B", buf, offset)[0]
        offset += 1
        num_unserved_drive_through = struct.unpack_from("<B", buf, offset)[0]
        offset += 1
        num_unserved_stop_go = struct.unpack_from("<B", buf, offset)[0]
        offset += 1
        grid_position = struct.unpack_from("<B", buf, offset)[0]
        offset += 1
        driver_status = struct.unpack_from("<B", buf, offset)[0]
        offset += 1
        result_status = struct.unpack_from("<B", buf, offset)[0]
        offset += 1
        pit_lane_timer_active = struct.unpack_from("<B", buf, offset)[0]
        offset += 1
        pit_lane_time = struct.unpack_from("<H", buf, offset)[0]
        offset += 2
        pit_stop_timer = struct.unpack_from("<H", buf, offset)[0]
        offset += 2
        pit_stop_should_serve_pen = struct.unpack_from("<B", buf, offset)[0]
        offset += 1
        speed_trap_fastest_speed = struct.unpack_from("<f", buf, offset)[0]
        offset += 4
        speed_trap_fastest_lap = struct.unpack_from("<B", buf, offset)[0]
        offset += 1
        ...
        laps.append(
            {
                ...
                "pitLaneTimeFormatted": f"{pit_lane_time / 1000:.3f}s"
                if pit_lane_time
                else "0.000s",
                "pitStopTimerInMS": pit_stop_timer,
                "pitStopTimeFormatted": f"{pit_stop_timer / 1000:.3f}s"
                if pit_stop_timer
                else "0.000s",
                ...
                if speed_trap_fastest_lap != 255
                else None,
            }
        )

    tt_pb = struct.unpack_from("<B", buf, offset)[0]
    offset += 1
    tt_rival = struct.unpack_from("<B", buf, offset)[0]
    offset += 1
