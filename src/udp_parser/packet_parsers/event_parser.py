...
from constants import EVENT_CODES, INFRINGEMENT_TYPES, PENALTY_TYPES
...
SAFETY_CAR_EVENT_TYPES = {0: "Deployed", 1: "Returning", 2: "Returned", 3: "Resume Race"}
...
def decode_event(buf: memoryview):
    ...
    """
    offset = _HDR_SIZE
    code = struct.unpack_from("<4s", buf, offset)[0]
    offset += 4
    ...
    if event_code == "FTLP":
        vehicle_idx = struct.unpack_from("<B", buf, offset)[0]
        offset += 1
        lap_time = struct.unpack_from("<f", buf, offset)[0]
        offset += 4
        details = {
        ...
        vehicle_idx = struct.unpack_from("<B", buf, offset)[0]
        offset += 1
        reason = struct.unpack_from("<B", buf, offset)[0]
        offset += 1
        details = {
            "vehicleIdx": vehicle_idx,
            "reason": reason,
        ...
        vehicle_idx = struct.unpack_from("<B", buf, offset)[0]
        offset += 1
        stop_time = struct.unpack_from("<f", buf, offset)[0]
        offset += 4
        details = {
            "vehicleIdx": vehicle_idx,
            "stopTime": stop_time,
        ...
        fields = struct.unpack_from("<BBBBBBB", buf, offset)
        offset += 7
        ...
        details = {
            ...
            "infringementTypeName": INFRINGEMENT_TYPES.get(
                infringement_type, f"Unknown ({infringement_type})"
            ),
            "vehicleIdx": fields[2],
        ...
        vehicle_idx = struct.unpack_from("<B", buf, offset)[0]
        offset += 1
        speed = struct.unpack_from("<f", buf, offset)[0]
        offset += 4
        is_overall = struct.unpack_from("<B", buf, offset)[0]
        offset += 1
        is_driver = struct.unpack_from("<B", buf, offset)[0]
        offset += 1
        fastest_idx = struct.unpack_from("<B", buf, offset)[0]
        offset += 1
        fastest_speed = struct.unpack_from("<f", buf, offset)[0]
        offset += 4
        details = {
            "vehicleIdx": vehicle_idx,
            "speedKph": speed,
        ...
        num_lights = struct.unpack_from("<B", buf, offset)[0]
        offset += 1
        details = {
        ...
        flash_frame = struct.unpack_from("<I", buf, offset)[0]
        offset += 4
        flash_time = struct.unpack_from("<f", buf, offset)[0]
        offset += 4
        details = {
        ...
        button_status = struct.unpack_from("<I", buf, offset)[0]
        offset += 4
        details = {"buttonFlags": button_status}
    elif event_code == "OVTK":
        overtak = struct.unpack_from("<B", buf, offset)[0]
        offset += 1
        overtaken = struct.unpack_from("<B", buf, offset)[0]
        offset += 1
        details = {"overtakingVehicleIdx": overtak, "beingOvertakenVehicleIdx": overtaken}
    elif event_code == "SCAR":
        safety_car_type = struct.unpack_from("<B", buf, offset)[0]
        offset += 1
        event_type = struct.unpack_from("<B", buf, offset)[0]
        offset += 1
        details = {
            "safetyCarType": safety_car_type,
            "safetyCarTypeName": SAFETY_CAR_TYPES.get(
                safety_car_type, f"Unknown ({safety_car_type})"
            ),
            "eventType": event_type,
            "eventTypeName": SAFETY_CAR_EVENT_TYPES.get(event_type, f"Unknown ({event_type})"),
        }
    elif event_code == "COLL":
        v1 = struct.unpack_from("<B", buf, offset)[0]
        offset += 1
        v2 = struct.unpack_from("<B", buf, offset)[0]
        offset += 1
    ...
    return {"eventCode": event_code, "eventName": event_name, "details": details}
