...
from constants import (SURFACE_TYPES, DRS_STATUS, MFD_PANELS, WHEEL_ORDER, 
                      TEMP_THRESHOLDS, SPEED_CONVERSIONS)
...
def decode_car_telemetry(buf: memoryview):
    """
    Decode F1 22 car telemetry packet with enhanced temperature warnings and surface information.
    """
    ...
    for car_idx in range(22):
        ...
        engine_temp_status = get_temp_status(engineTemp, 
                                           TEMP_THRESHOLDS["engine_warning"], 
                                           TEMP_THRESHOLDS["engine_critical"])
        
        brake_temp_status = [get_temp_status(temp, 
                                           TEMP_THRESHOLDS["brake_warning"], 
                                           TEMP_THRESHOLDS["brake_critical"]) 
                           for temp in brakesTemp]
        
        tyre_temp_status = [get_temp_status(temp, 
                                          TEMP_THRESHOLDS["tyre_warning"], 
                                          TEMP_THRESHOLDS["tyre_critical"]) 
        ...
        car_data = {
            ...
            "surfaceByWheel": {
                WHEEL_ORDER[i]: surface_names[i] for i in range(4)
            },
            
            # Warning flags
            "hasEngineWarning": engine_temp_status in ["WARNING", "CRITICAL"],
            "hasBrakeWarning": any(status in ["WARNING", "CRITICAL"] for status in brake_temp_status),
    ...
    return {
        ...
        "mfdPanelNameSecondary": MFD_PANELS.get(m_mfdPanelIndexSecondary, f"Unknown ({m_mfdPanelIndexSecondary})"),
        "suggestedGear": m_suggestedGear,
        "suggestedGearDisplay": format_gear(m_suggestedGear) if m_suggestedGear != 0 else "No suggestion"
    }
