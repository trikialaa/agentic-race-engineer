...
class TelemetryDisplay:
    def __init__(self, stdscr):
        ...
        self.data = {
            "session": {
                ...
                "trackTemperature": 0,
                "airTemperature": 0,
                "sessionTimeLeftFormatted": "00:00",
            },
            "lap_data": {
                "laps": [
                    {
                        "carPosition": 0,
                        "currentLapNum": 0,
                        "lastLapTimeFormatted": "0:00.000",
                        "sector1TimeFormatted": "00.000",
                        "sector2TimeFormatted": "00.000",
                        "driverStatusName": "Unknown",
                        "resultStatusName": "Unknown",
                    }
                ]
            },
            "car_telemetry": {
                ...
                "carStatus": [
                    {
                        "fuelPercentage": 0,
                        "fuelRemainingLaps": 0,
                        "fuelMixName": "Standard",
                        "ersPercentage": 0,
                        "ersDeployModeName": "None",
                        "actualTyreCompoundName": "Unknown",
                        "tyresAgeLaps": 0,
                        "drsStatus": "Not allowed",
                        "flagColor": "None",
                    }
                ]
            },
        ...
        if packet_type == "event":
            ...
            if event_code:
                ...
                if event_code and event_code != "NULL":
                    self.data[packet_type]["eventHistory"].insert(
                        0,
                        {"code": event_code, "details": details, "time": time.strftime("%H:%M:%S")},
                    )
                    # Keep only the last 5 non-null events
                    self.data[packet_type]["eventHistory"] = [
                        evt
                        for evt in self.data[packet_type]["eventHistory"][:5]
    ...
    def display_race_standings(self, start_y: int) -> int:
        ...
        self.stdscr.addstr(
            start_y,
            0,
            f"=== F1 RACE MONITOR === {session.get('trackName', 'Unknown')} | {session.get('sessionTypeName', 'Unknown')} | {session.get('weatherName', 'Clear')}",
        ...
        self.stdscr.addstr(
            start_y,
            0,
            f"Track: {session.get('trackTemperature', 0)}°C | Air: {session.get('airTemperature', 0)}°C | Time: {session.get('sessionTimeLeftFormatted', '00:00')}",
        )
        start_y += 2

        # Race standings header
        self.stdscr.addstr(
            start_y, 0, "=== RACE STANDINGS ===", curses.A_BOLD | curses.color_pair(5)
        )
        start_y += 1
        self.stdscr.addstr(
            start_y,
            0,
            "Pos Driver           Team           Lap  Last Lap    S1     S2     Status",
            curses.A_BOLD,
        )
        start_y += 1
        ...
        if self.data["participants"]["participants"] and self.data["lap_data"]["laps"]:
            ...
            for i in range(min(len(participants), len(lap_data), 22)):
                ...
                if participant.get("displayName") or participant.get(
                    "name"
                ):  # Only show active drivers
                    driver_name = participant.get(
            ...
            max_drivers = min(
                len(drivers_data), max_y - start_y - 15
            )  # Leave space for player info
    ...
    def display_player_details(self, start_y: int) -> int:
        ...
        if self.data["car_telemetry"]["carTelemetry"]:
            tel = self.data["car_telemetry"]["carTelemetry"][
                min(self.player_car_index, len(self.data["car_telemetry"]["carTelemetry"]) - 1)
            ]
            self.stdscr.addstr(
                start_y, 0, "=== PLAYER CAR ===", curses.A_BOLD | curses.color_pair(5)
            )
            start_y += 1
            ...
            rpm_color = (
                curses.color_pair(1)
                if tel.get("engineRPM", 0) < 10000
                else curses.color_pair(2)
                if tel.get("engineRPM", 0) < 12000
                else curses.color_pair(3)
            )
            drs_color = (
                curses.color_pair(1)
                if tel.get("drsStatus", "Off") != "Off"
                else curses.color_pair(0)
            )
            ...
            self.stdscr.addstr(
                start_y,
                0,
                f"Speed: {tel.get('speedKph', 0):3d}km/h | Gear: {tel.get('gearDisplay', 'N'):>2} | ",
            )
            self.stdscr.addstr(f"RPM: {tel.get('engineRPM', 0):5d}", rpm_color)
            self.stdscr.addstr(" | DRS: ")
            ...
            self.stdscr.addstr(
                start_y,
                0,
                f"Throttle: {tel.get('throttlePercent', 0):3.0f}% | Brake: {tel.get('brakePercent', 0):3.0f}% | Engine: {tel.get('engineTemperature', 0):3d}°C",
            )
            start_y += 1

        # Car status
        if self.data["car_status"]["carStatus"]:
            status = self.data["car_status"]["carStatus"][
                min(self.player_car_index, len(self.data["car_status"]["carStatus"]) - 1)
            ]
            ...
            self.stdscr.addstr(start_y, 0, "Fuel: ")
            ...
            self.stdscr.addstr(
                f" | Tyres: {status.get('actualTyreCompoundName', 'Unknown')} ({status.get('tyresAgeLaps', 0)} laps)"
            )
            start_y += 1

        # Damage summary
        if self.data["car_damage"]["carDamage"]:
            damage = self.data["car_damage"]["carDamage"][
                min(self.player_car_index, len(self.data["car_damage"]["carDamage"]) - 1)
            ]
            wear = damage.get("m_tyresWear", [0, 0, 0, 0])
            max_wear = max(wear) if wear else 0
            wing_dmg = max(
                damage.get("m_frontLeftWingDamage", 0),
                damage.get("m_frontRightWingDamage", 0),
                damage.get("m_rearWingDamage", 0),
            )
            ...
            self.stdscr.addstr(start_y, 0, "Tyre Wear: ")
            self.stdscr.addstr(f"Max {max_wear:.0f}%", wear_color)
            self.stdscr.addstr(" | Wing Damage: ")
    ...
    def display_recent_events(self, start_y: int) -> int:
        ...
        self.stdscr.addstr(
            start_y, 0, "=== RECENT EVENTS ===", curses.A_BOLD | curses.color_pair(5)
        )
        start_y += 1
        ...
        if self.data["event"]["eventHistory"]:
            for i, event in enumerate(self.data["event"]["eventHistory"][:3]):  # Show max 3 events
                if event.get("code") and event["code"] != "NULL":
                    ...
                    color = (
                        curses.color_pair(2)
                        if "PENA" in event["code"]
                        else curses.color_pair(1)
                        if "FTLP" in event["code"]
                        else curses.color_pair(0)
                    )
    ...
    def display(self):
        ...
        try:
            # Title (1 line)
            self.stdscr.addstr(
                0, 0, "F1 25 RACE MONITOR - Press 'q' to quit", curses.A_BOLD | curses.color_pair(4)
            )
            ...
            if max_y > current_y + 3:
                ...
                stats1 = " | ".join(
                    [f"{packet_abbrevs.get(k, k[:3].upper())}: {v}" for k, v in items[:mid]]
                )
                stats2 = " | ".join(
                    [f"{packet_abbrevs.get(k, k[:3].upper())}: {v}" for k, v in items[mid:]]
                )
